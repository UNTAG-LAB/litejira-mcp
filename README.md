...
- **本工具只能分類與止血**：無法確認上游根因，也無法讓結果變回來。重跑有機會過，
  但不保證。
- **寫入動作請先查現況再重送**：無法確認指令碼是否已執行完，留言／連結／狀態變更
  仍可能已生效，只是結果取不回來。伺服器端不對 `idempotencyKey` 去重，
  直接重送有可能寫第二遍。錯誤訊息會標示「寫入仍可能已生效」，並帶 `transient=false`
  擋掉呼叫端的自動重試。
